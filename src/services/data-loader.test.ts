...
describe('Data Loader Service', () => {
  ...
  describe('loadTracksDatabase()', () => {
    ...
    it('應該在 fetch 失敗時拋出錯誤', async () => {
      ...
      // Act & Assert
      await expect(loadTracksDatabase()).rejects.toThrow('Failed to load tracks database');
    });
  ...
  describe('checkDataIntegrity()', () => {
    it('應該檢測重複的 track ID', () => {
      ...
      expect(report.issues.some((issue) => issue.type === 'DUPLICATE_TRACK_ID')).toBe(true);
    });
    ...
    it('應該檢測 metadata 與實際資料不一致', () => {
      ...
      // Assert
      expect(report.isValid).toBe(false);
      expect(report.issues.some((issue) => issue.type === 'METADATA_MISMATCH')).toBe(true);
    });
