...
- Q: 對於已分享的舊連結（如 `/spotify-youtube-hits/track/123`），系統應如何處理？ → A: 不處理：舊連結將失效，使用者需使用新的根路徑 URL 格式
...
- **How does system handle** 舊版 URL 結構？
  - 舊版 URL 路徑（`/spotify-youtube-hits/*`）將不再有效，使用者需使用新的根路徑 URL 格式（`/*`）。已分享的舊連結將無法正常運作
...
6. **URL 結構**: 假設應用的 URL 路由結構從 `/spotify-youtube-hits/*` 變更為 `/*`（根路徑），需要處理舊連結的相容性
...
2. **URL 路由變更導致舊連結失效**
   - **影響**: 中 - 已分享的舊連結（`/spotify-youtube-hits/*`）將無法正常運作
